...
resource "aws_vpc_endpoint" "s3_endpoint" {
  ...
  policy          = var.s3_endpoint_policy == null ? templatefile("${path.module}/templates/default_gateway_endpoint_policy.json.tpl", {}) : var.s3_endpoint_policy
}
...
resource "aws_vpc_endpoint" "dynamo_endpoint" {
  ...
  route_table_ids = var.use_nat_gateway ? [aws_route_table.private_nat_gateway[count.index].id] : [aws_route_table.private_nat_instance[count.index].id]
  policy          = var.dynamo_endpoint_policy == null ? templatefile("${path.module}/templates/default_gateway_endpoint_policy.json.tpl", {}) : var.dynamo_endpoint_policy
}
